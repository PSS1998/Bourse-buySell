...
def process_stock(ticker):
	...
	with open("../bourse_price/"+ticker, 'rb') as fd:
		...
		ema100 = ta.EMA(np.array(df['<CLOSE>'].astype(float)), timeperiod=50)

		if( (np.array(df['<CLOSE>'])[-1] < ema100[-1]) &
		(np.array(df['<CLOSE>'])[-1] < lowerband[-1]) ):
			return "buy"
		if( np.array(df['<CLOSE>'])[-1] > middleband[-1] ):
			return "sell"
